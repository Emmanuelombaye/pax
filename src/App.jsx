...
function App() {
  ...
  useEffect(() => {
    const handleHashChange = () => {
      ...
      setIsNavOpen(false); // Close mobile nav on transition
  ...
  const handleQuizOptionSelect = (key, value) => {
  ...
  const isNextDisabled = () => {
    if (quizStep === 1 && !quizData.goal) return true;
        ...
        <div className="container">
          <a href="#/" className="logo">
            <div className="logo-wrapper">
              <img src="/logo.png" alt="Pax Logo" className="logo-icon" />
          ...
          <div className="fade-in">
            ...
            <section className="cinematic-hero">
              <div className="slideshow-container">
                {[
                  '/images/hero-longevity.png',
                  '/images/miami-active.png',
                  '/images/hero-miami-cycle.png',
                  '/images/hero-miami-yoga.png',
                  '/images/hero-miami-water.png'
                ].map((imgUrl, index) => (
                  <div 
                    key={index}
      ...
      <div className={`modal ${isQuizOpen ? 'open' : ''}`}>
        ...
        <div className="modal-container">
          <div className="modal-header">
            <h3 className="modal-title">Longevity Health Intake</h3>
            <button className="modal-close" onClick={closeQuiz} aria-label="Close Intake">&times;</button>
          </div>
            ...
            <form onSubmit={handleQuizSubmit}>
              ...
              {quizStep === 1 && (
                <div className="quiz-step active">
                  <h4 className="quiz-question">What is your primary biological health and wellness goal?</h4>
                  <div className="quiz-options">
                    {[
                      { val: 'weight-loss', label: 'Clinical Weight Management (Semaglutide/GLP-1)' },
                      { val: 'cellular-energy', label: 'Cellular Longevity & Mitochondrial Energy (NAD+)' },
                      { val: 'muscle-recovery', label: 'Muscle Recovery, Vitality & Sleep (Sermorelin)' }
                    ].map((opt) => (
                      <button 
                        key={opt.val}
                        type="button" 
                        className={`quiz-option ${quizData.goal === opt.val ? 'selected' : ''}`}
                        onClick={() => handleQuizOptionSelect('goal', opt.val)}
                      >
                        {opt.label}
                      </button>
                    ))}
              ...
              {/* Step 2: Weight/BMI Check */}
              {quizStep === 2 && (
                <div className="quiz-step active">
                  <h4 className="quiz-question">Please share your approximate height and weight.</h4>
              ...
              {quizStep === 3 && (
                <div className="quiz-step active">
                  <h4 className="quiz-question">Do you have any pre-existing medical conditions (thyroid issues, diabetes, heart conditions)?</h4>
                  <div className="quiz-options">
                    {[
                      { val: 'none', label: 'No pre-existing conditions' },
                      { val: 'thyroid', label: 'Personal/Family history of thyroid cancer (MTC)' },
                      { val: 'diabetes', label: 'Diabetes or glycemic regulation conditions' },
                      { val: 'other', label: 'Other major conditions (Cardio, Renal)' }
                    ].map((opt) => (
                      <button 
                        key={opt.val}
                        type="button" 
                        className={`quiz-option ${quizData.condition === opt.val ? 'selected' : ''}`}
                        onClick={() => handleQuizOptionSelect('condition', opt.val)}
                      >
                        {opt.label}
                      </button>
                    ))}
              ...
              {quizStep === 4 && (
                <div className="quiz-step active">
                  <h4 className="quiz-question">Where should your clinical provider send your assessment results?</h4>
                  <input 
                    type="text" 
                    className="quiz-input-text" 
                    value={quizData.name}
